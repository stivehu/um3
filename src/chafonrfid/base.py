...
class ReaderResponseFrame(object):
    ...
    def __init__(self, resp_bytes, offset=0):
        ...
        if self.len + offset > len(resp_bytes) - 1:
            raise ValueError('Response does not contain enough bytes for frame '
                             '(expected %d bytes after offset %d, actual length %d)'
    ...
    def verify_checksum(self, data_bytes, checksum_bytes):
        ...
        return checksum_bytes[0] == crc_lsb \
               and checksum_bytes[1] == crc_msb
...
class ReaderInfoFrame(ReaderResponseFrame):
    ...
    def __init__(self, resp_bytes):
        ...
        if len(self.data) >= 8:
            ...
            self.frequency_band = ReaderFrequencyBand(((dmaxfre & 0b11000000) >> 4) +
                                                      ((dminfre & 0b11000000) >> 6))
...
class TagData(object):
    ...
    def get_tag_data(self):
        ...
        while n < self.num_tags:
            ...
            yield (self.data[tag_data_start:tag_main_start],
                   self.data[tag_main_start:tag_main_end],
